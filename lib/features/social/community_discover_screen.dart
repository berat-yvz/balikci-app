...
/// Kayıtlı balıkçıları ara ve keşfet — eski Topluluk sekmesinin içeriği.
///
/// [showShortcutRow] false ise üstteki Arkadaşlarım / İstekler kısayolları gizlenir
/// ([FriendsHubScreen] içinde Liste sekmesi zaten var).
class CommunityDiscoverScreen extends ConsumerStatefulWidget {
  const CommunityDiscoverScreen({
    super.key,
    this.showShortcutRow = true,
  });

  final bool showShortcutRow;
...
class _CommunityDiscoverScreenState extends ConsumerState<CommunityDiscoverScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: [
        if (widget.showShortcutRow && current != null) ...[
          SliverToBoxAdapter(
            child: Padding(
          ...
          discoverAsync.when(
            ...
            error: (e, _) => SliverToBoxAdapter(
              child: NetworkErrorWidget(
                ...
                onRetry: () => ref.invalidate(allRegisteredAnglersProvider),
              ),
            ),
          ),
        ],
                              ...
                              SnackBar(
                                content: Text(
                                  ErrorMessageHelper.toUserMessage(
                                    e,
                                    fallback: 'İstek kabul edilemedi.',
                                  ),
                                ),
                              ),
                            );
